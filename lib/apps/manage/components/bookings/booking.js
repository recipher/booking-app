...
export default State('booking')(({ booking, last, dispatch, $, state }) => {
  ...
  return (
    <Row onMouseOver={events.over} onMouseOut={events.out} last={last}>
      <Cell width='45%'>
        <strong>{booking.data.brand.name} {booking.data.product.name}</strong>
      </Cell>
      <Cell width='10%'>
        <span style={{fontSize:'0.9em'}}>{booking.data.model.name}</span>
      </Cell>
      <Cell width='20%'><DateTime dt={booking.slot} format='ddd h:mma' /></Cell>
      <Cell width='25%'>
        <span title={booking.member}>
          {booking.info.name} {booking.info.surname}
        </span>
      </Cell>
    </Row>
  );
