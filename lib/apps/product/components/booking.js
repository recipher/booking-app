...
const LIMIT = 2
    , LOCKED_AT = moment(new Date(2017, 5, 8));
...
export const Booking = Radium((props) => {
  ...
  const inCart = find(cart.data, b => b.slot.index === slot.index)
      ...
      , locked = moment().isAfter(LOCKED_AT)
      , bookable = !limited && !booking && !inCart && !locked;

  return (
    <div onClick={_ => bookable && select(slot)} style={[ styles.base, (limited || booking || locked) && styles.booked, own && styles.own ]}>
