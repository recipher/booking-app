...
export const Header = _ => {
  return (
    <Row kind='heading'>
      <Cell width='30%'>Bike</Cell>
      <Cell width='20%'>Size</Cell>
      <Cell width='40%'>Slot</Cell>
      <Cell width='10%' align='right'><br/></Cell>
    </Row>
  );
};

export const Booking = State('booking')(({ booking, product, brand, id, last, dispatch, $, state }) => {
  ...
  return (
    <Row onMouseOver={events.over} onMouseOut={events.out} last={last}>
      ...
      <Cell width='20%'>{booking.model.name}</Cell>
      <Cell width='40%'>
        {moment(booking.slot.slot).format('ddd MMM Do, h:mma')}
      </Cell>
      <Cell width='10%' align='right'>Remove</Cell>
    </Row>
  );
});
