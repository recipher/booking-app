...
export default (state = initialState, action) => {
  ...
  case BOOK:
    return assign({}, state, { paying: true, error: null });

  case SAVE_SUCCESS:
    const data = map(reject(state.data, booking => {
    ...
    }), booking => assign({}, booking, { error: 'taken'}));

    return assign({}, state, { saving: false, saved: true, paying: false, data, error: null });

  case SAVE_FAILED:
    return assign({}, state, { saving: false, saved: true, paying: false, error: ERRORS[action.payload.status] });
